...
# Parse the control file for a package source name.
# 
DEBDIR=$(git rev-parse --show-toplevel)/debian
if [ -e ${DEBDIR}/control ]; then
	DEBSOURCE=$(sed -n 's/Source:\s*//p' ${DEBDIR}/control)
else
	DEBSOURCE=$(basename $(git remote get-url origin) | cut -d. -f1)
fi
