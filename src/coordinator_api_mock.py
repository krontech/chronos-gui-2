...
class ControlAPIMock(QObject):
	...
	def emitSignal(self, signalName: str, *args) -> None:
		...
		signal = QDBusMessage.createSignal('/ca/krontech/chronos/coordinator_mock', 'ca.krontech.chronos.coordinator_mock', signalName)
		for arg in args:
			signal << arg
	...
	def emitControlSignal(self, name: str, value=None) -> None:
		...
		signal = QDBusMessage.createSignal('/ca/krontech/chronos/coordinator_mock', 'ca.krontech.chronos.coordinator_mock', name)
...
if not QDBusConnection.systemBus().registerService('ca.krontech.chronos.coordinator_mock'):
...
QDBusConnection.systemBus().registerObject('/ca/krontech/chronos/coordinator_mock', controlAPI, QDBusConnection.ExportAllSlots)
