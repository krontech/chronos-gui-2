...
class Hardware():
	...
	def __init__(self, app):
		...
		self._jogWheelEncoderA   = os.fdopen(os.open(gpioPath/"gpio20/value", os.O_RDONLY | os.O_NONBLOCK, 0), 'rb', buffering=0)
		self._jogWheelEncoderB   = os.fdopen(os.open(gpioPath/"gpio26/value", os.O_RDONLY | os.O_NONBLOCK, 0), 'rb', buffering=0)
		...
		#Pump events. Jog wheel can change very often, and has more state, so it has to be checked more frequently.
		self.highResolutionTimer = QTimer()
		self.highResolutionTimer.timeout.connect(self.__updateJogWheelRotation)
		self.highResolutionTimer.start(1) #ms
	...
	def __updateJogWheelRotation(self):
		"""Read the jog wheel encoder and update internal state accordingly.
		
			Since the encoder FIFO seems to be one element long, this
			function needs to be called frequently enough that we
			don't skip over any intermediate rotations.
			
			The way the physical jog wheel works is that has two
			encoders on it, each reporting high (1) or low (0).
			They're offset, so depending on what the state of one is
			when the other changes, you know which direction the wheel
			is turning.
			
			This function was ported from the camApp."""
		
		#Jog wheel encoder in detent with EncA = 1, EncB = 1. Clockwise rotation results in first change being EncA <- 0;
		self._jogWheelEncoderA.seek(0)
		self._jogWheelEncoderB.seek(0)
		jogWheelEncoderA = True if self._jogWheelEncoderA.read(1) == b'1' else False
		jogWheelEncoderB = True if self._jogWheelEncoderB.read(1) == b'1' else False
