...
class ControlAPI(QObject):
	...
	def emitControlSignal(self, name: str, value=None) -> None:
		...
		signal = QDBusMessage.createSignal('/ca/krontech/chronos/coordinator', 'ca.krontech.chronos.coordinator', name)
		signal << getattr(state, name) if value is None else value
		QDBusConnection.systemBus().send(signal)
	
	def emitError(self, message: str) -> None:
		error = QDBusMessage.createErrorReply(QDBusError.Other, message)
	...
	def framerateForResolution(self, hRes: int, vRes: int):
...
if not QDBusConnection.systemBus().registerService('ca.krontech.chronos.coordinator'):
...
QDBusConnection.systemBus().registerObject('/ca/krontech/chronos/coordinator', controlAPI, QDBusConnection.ExportAllSlots)
